...
export default function App() {
  ...
  const handleQuery = async (query: string, typeHint: string = 'general') => {
    ...
    try {
      // Snappy typing delay (100ms) for smooth transition without artificial lag
      const minDelayPromise = new Promise(resolve => setTimeout(resolve, 100));
          ...
          <div className="flex-1">
            <AnimatePresence>
              {viewState === 'landing' && (
                <motion.a
                  initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  href="https://drive.google.com/file/d/1kHftz-ThPEwj6CSMdN5IeEqGELA8Wu4k/view?usp=sharing"
                  target="_blank"
